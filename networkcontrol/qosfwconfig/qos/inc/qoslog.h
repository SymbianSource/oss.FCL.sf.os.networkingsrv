...
_LIT(KQoSLogFolder,"qos");
_LIT(KQoSLogFile,"qos.txt");  
...
{
	RFileLogger::Write(KQoSLogFolder(), KQoSLogFile(), EFileLoggingModeAppend, aDes);
}
...
inline void Log::Printf(TRefByValue<const TDesC> aFmt, ...)
{
	VA_LIST list;
	VA_START(list,aFmt);
	RFileLogger::WriteFormat(KQoSLogFolder(), KQoSLogFile(), EFileLoggingModeAppend, aFmt, list);
}
...
{
	RFileLogger::HexDump(KQoSLogFolder(), KQoSLogFile(), EFileLoggingModeAppend, aHeader, aMargin, aPtr, aLen);
}
