...
_LIT(KQosLibLogFolder,"qoslib");
_LIT(KQosLibLogFile,"qoslib.txt");  
...
{
	RFileLogger::Write(KQosLibLogFolder(), KQosLibLogFile(), EFileLoggingModeAppend, aDes);
}
...
inline void Log::Printf(TRefByValue<const TDesC> aFmt, ...)
{
	VA_LIST list;
	VA_START(list,aFmt);
	RFileLogger::WriteFormat(KQosLibLogFolder(), KQosLibLogFile(), EFileLoggingModeAppend, aFmt, list);
}
...
{
	RFileLogger::HexDump(KQosLibLogFolder(), KQosLibLogFile(), EFileLoggingModeAppend, aHeader, aMargin, aPtr, aLen);
}

#endif // !defined(_DEBUG)

#endif //__QOSLIBLOG_H__
